...
function decide_response(user_said) {
  ...
  var play_re = /ちゅうもん\s(.+)/i;  // creating a regular expression
  ...
  } else if (user_said.toLowerCase().includes("さようなら")) {
    response = "じゃあ, また　ね!";
    state = "initial"

  } else {
    response = "もう一いちどいってください";
  }
  return response;
}
...
function speak(text, callback) {
  ...
  u.voice = speechSynthesis.getVoices().filter(function(voice) { return voice.name == "Kyoko"; })[0]; 
